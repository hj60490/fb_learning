...
def test_get_all_posts_with_posts_return_posts_dto(users, posts,
                                                   posts_details_dto,
...
@pytest.mark.django_db
def test_get_all_posts_with_no_posts_return_empty(users, request_parameters_dto):
    user_id = 1
    expected_output = []
    post_storage = PostStorageImplementation()
    actual_output = post_storage.get_posts(user_id=user_id,
...
@pytest.mark.django_db
def test_get_all_reaction_on_post_return_reaction_dto(users, posts, reaction_details_dto, reacts):
    posts_id = [1]
...
@pytest.mark.django_db
def test_get_all_comments_on_post_return_comment_dto(users, posts, comments_details_dto, comments):
    posts_id = [1]
...
@pytest.mark.django_db
def test_get_reactions_on_comments_return_reaction(users, posts, comments, reacts_on_comments, reaction_on_comments_details_dto):
    comments_id = [1]
